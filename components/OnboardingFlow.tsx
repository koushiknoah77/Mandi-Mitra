...
export const OnboardingFlow: React.FC<OnboardingFlowProps> = ({ onComplete }) => {
  ...
  // Onboarding Voice Commands
  const onboardingCommands = useMemo<VoiceCommand[]>(() => {
    const commands: VoiceCommand[] = [];

    if (step === 'welcome') {
      commands.push({ id: 'start', keywords: ['start', 'begin', 'trading', 'go'], callback: handleStart, description: 'Start' });
    } else if (step === 'language') {
      Object.values(SUPPORTED_LANGUAGES).forEach(lang => {
        commands.push({
          id: `lang-${lang.code}`,
          keywords: [lang.nativeName, lang.name, lang.code],
          callback: () => handleLanguageSelect(lang.code),
          description: `Select ${lang.name}`
        });
      });
    } else if (step === 'role') {
      commands.push(
        { id: 'role-seller', keywords: ['farmer', 'sell', 'kisan', 'vikri', 'bech'], callback: () => handleRoleSelect(UserRole.SELLER), description: 'Farmer' },
        { id: 'role-buyer', keywords: ['buyer', 'buy', 'trader', 'vyapari', 'kharedi'], callback: () => handleRoleSelect(UserRole.BUYER), description: 'Buyer' }
      );
    }
  ...
  useRegisterVoiceCommands(onboardingCommands);

  const handleVoiceInteraction = async () => {
    if (globalVoiceState.isListening) { stopGlobalListen(); return; }

    const transcript = await startGlobalListen();
    if (!transcript) return;

    // Local handling for the 'Details' step which is dynamic AI extraction
    if (step === 'details') {
      ...
      <div className="relative z-10 w-full max-w-[480px]">
        <div className="bg-white rounded-[40px] shadow-[0_20px_50px_-12px_rgba(0,0,0,0.06)] p-8 sm:p-12 transition-all duration-500 ease-out min-h-[640px] flex flex-col justify-between border border-slate-100">
          ...
          <div className="flex justify-center pt-6 relative z-10">
            <VoiceIndicator
              state={step === 'details' && isProcessingVoice ? { ...globalVoiceState, isProcessing: true } : globalVoiceState}
